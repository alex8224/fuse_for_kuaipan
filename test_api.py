...
def main(consumer_key, consumer_secret, app_name, username, userpwd, url):
    ...
    # print api.delete("testupload/newfolder")
    # testupload_files = api.metadata(path="testupload")
    # for remotefile in testupload_files["files"]:
        # print "move %s to /" % remotefile["name"]
        # print api.move("testupload/%s" % remotefile["name"],"/testupload2/%s" % remotefile["name"])
        # print "move %s to / ok" % remotefile["name"]

    # from timeit import timeit
    # print timeit(timed,number=1)
    # import pdb;pdb.set_trace()
    try:
        n = 0
        fd = StreamIO()
        for data in api.download_file("/Symfony_book_2.4.pdf"):
            n+=len(data)
            fd.write(data)
            print n
        ...
        import traceback;traceback.print_exc()
    # api.download_file("testupload/pa.mp4")
    # api.download_file("testupload/OSGi_in_Action_v1_PUBLISH.pdf")
    # api.create_folder("中文谜目")
    # api.download_file("中文谜目/Contrail-OVFtoONE_v0.1.pdf")
    # print api.upload("testupload", "/home/alex/storage/download/Power-All中文版.mp4")
...
if __name__ == "__main__":
    ...
    fuse = FUSE(KuaiPanFuse(), argv[1], foreground=True, nothreads=False, debug=True)
